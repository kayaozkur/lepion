...
cat > "$OUTPUT_FILE" << EOF
{
  ...
  "name": "$SITE_NAME",
  "logo": {
    "dark": "/brand_assets/logo/svg/brand_logo/dark.svg",
    "light": "/brand_assets/logo/svg/brand_logo/light.svg"
  },
  "favicon": "/brand_assets/logo/svg/icon/dark.svg",
  "colors": {
...
# Check if there are any top-level directories for tabs (excluding brand_assets)
HAS_DIRS=false
if find "$DOCS_DIR" -mindepth 1 -maxdepth 1 -type d -not -path "*/.*" -not -name "brand_assets" | grep -q .; then
    HAS_DIRS=true
    echo '  "tabs": [' >> "$OUTPUT_FILE"
    
    find "$DOCS_DIR" -mindepth 1 -maxdepth 1 -type d -not -path "*/.*" -not -name "brand_assets" | sort | while read -r dir; do
        dirname=$(basename "$dir")
        tab_name=$(filename_to_title "$dirname")
        echo "    {" >> "$OUTPUT_FILE"
...
# Generate navigation for each top-level directory (excluding brand_assets)
find "$DOCS_DIR" -mindepth 1 -maxdepth 1 -type d -not -path "*/.*" -not -name "brand_assets" | sort | while read -r dir; do
    dirname=$(basename "$dir")
    group_title=$(filename_to_title "$dirname")
    ...
    find "$dir" -name "*.md" -o -name "*.mdx" | sort | while read -r file; do
        page_path=$(get_page_path "$file" "$DOCS_DIR")
...
total_dirs=$(find "$DOCS_DIR" -mindepth 1 -maxdepth 1 -type d -not -path "*/.*" -not -name "brand_assets" | wc -l | tr -d ' ')
